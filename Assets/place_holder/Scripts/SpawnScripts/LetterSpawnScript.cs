    ...
    public void spawnLetters(bool spawn_allowed, int number) 
    {
        if(spawn_allowed) {
            for (int i = 0; i < number; i++)
            {
                ...
                random_coordinates_z = Random.Range(-0.15f, 0.15f);
                GameObject letters = Instantiate(letter, letter_spawn_points[random_spawn_point].position + new Vector3(random_coordinates_x,0,random_coordinates_z), Quaternion.Euler(90,0,0));
